...
const CIRCLE_SIZE: [(Number, bool); 10] = [
    ...
    (89.0, false), // 4 CIRCLE INVISIBLE -
    (92.0, false), // 5 CIRCLE INVISIBLE PLANET
    //    (0.0, false), // 5
    (0.0, false),  // 6
    (82.0, false), // 7 between 2 and 3
    (90.0, false), // 8 correction planet between 2 and 3
    (43.0, false), // 9 Planet pos transit
];
...
#[derive(Debug, Clone)]
pub struct WorkingStorageDrawTransit {
    ws: WorkingStorageTransit,
}
...
impl WorkingStorageDrawTransit {
    pub fn new(ws: WorkingStorageTransit) -> WorkingStorageDrawTransit {
        WorkingStorageDrawTransit { ws: ws }
    }
}

// Methods

impl DrawTransit for WorkingStorageDrawTransit {
    fn draw_base(&self) -> Document {
...
impl CalcDrawTransit for WorkingStorageTransit {
    fn get_radius_total(&self) -> Number {
        self.max_size / 2.0
    }
    ...
    fn get_radius_rules_inside_circle(
        ...
        self.get_radius_total()
            * (((CIRCLE_SIZE[2].0 - CIRCLE_SIZE[1].0) / size)
                + CIRCLE_SIZE[1].0)
            / 100.0
    }
    ...
    fn get_radius_rules_inside_circle_house_for_pointer_bottom(
        ...
        (self.get_radius_total()
            * (((CIRCLE_SIZE[3].0 - CIRCLE_SIZE[2].0) / div_trait_pointer)
                - CIRCLE_SIZE[3].0))
            / 100.0
    }
    ...
    fn get_radius_rules_inside_circle_house_for_pointer_top(&self) -> Number {
        (self.get_radius_total()
            * ((CIRCLE_SIZE[3].0 - CIRCLE_SIZE[2].0) - CIRCLE_SIZE[3].0))
            / 100.0
    }
    ...
    fn get_radius_circle_zodiac(&self) -> Number {
        ...
        (self.get_radius_total()
            * (((CIRCLE_SIZE[2].0 - CIRCLE_SIZE[1].0) / (2.0 + div_trait_big))
                + CIRCLE_SIZE[1].0))
            / 100.0
    }
    ...
    fn get_radius_circle_house(&self) -> Number {
        (self.get_radius_total()
            * (((CIRCLE_SIZE[3].0 - CIRCLE_SIZE[2].0) / 2.0)
                + CIRCLE_SIZE[2].0))
            / 100.0
    }
