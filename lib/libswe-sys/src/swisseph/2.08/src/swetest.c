/*
  $Header: /users/dieter/sweph/RCS/swetest.c,v 1.78 2010/06/25 07:22:10 dieter
Exp $ swetest.c	A test program
    ...
    *stimein = '\0';
    ...
    for (i = 1; i < argc; i++) {
        if (strncmp(argv[i], "-utc", 4) == 0) {
            ...
            sp = argv[i] + 4;
            if (*sp == '[') sp++;
            sscanf(sp, "%lf,%lf,%lf,%lf,%lf,%lf", &(dobs[0]), &(dobs[1]),
                   &(dobs[2]), &(dobs[3]), &(dobs[4]), &(dobs[5]));
        }
            ...
            sp = argv[i] + 2;
            if (*sp == 'c' || *sp == '\0') {
                // char si0[strlen(infocmd0)+1]; // Microsoft Visual Studio does
                // not like this
                char si0[2000];
                swe_version(sout);
                strcpy(si0, infocmd0);
    ...
    if (special_event == SP_OCCULTATION || special_event == SP_RISE_SET ||
        ...
        if (*plsel == 'f') {
            ...
            if (*plsel == 's') ipl = atoi(sastno) + 10000;
            *star = '\0';
        }
    ...
    while (TRUE) {
        ...
        if (*sp == 'j') { /* it's a day number */
            ...
            if ((int32)jyear * 10000L + (int32)jmon * 100L + (int32)jday <
                15821015L)
                gregflag = SE_JUL_CAL;
            else
                gregflag = SE_GREG_CAL;
        ...
        if (special_event > 0) {
            do_special_event(tjd, ipl, star, special_event, special_mode,
                             geopos, datm, dobs, serr);
            swe_close();
            return OK;
        }
        line_count = 0;
        for (t = tjd, istep = 1; istep <= nstep; t += tstep, istep++) {
            ...
            if (step_in_years) {
                swe_revjul(tjd, gregflag, &jyear, &jmon, &jday, &jut);
                t = swe_julday(jyear + (istep - 1) * (int)tstep, jmon, jday,
                               jut, gregflag);
            }
            if (step_in_months) {
                swe_revjul(tjd, gregflag, &jyear, &jmon, &jday, &jut);
            ...
            if (with_header) {
                ...
                if (do_houses) {
                    ...
                    if (!universal_time) {
                        do_houses = FALSE;
                        printf(
                            "option -house requires option -ut for Universal "
                            "Time\n");
                    }
                    else {
	...
	fputs("\n", stdout);
            ...
            for (psp = plsel; *psp != '\0'; psp++) {
                ...
                if (iflag & SEFLG_HELCTR) {
                    if (ipl == SE_SUN || ipl == SE_MEAN_NODE ||
                ...
                if (iflgret < 0) {
                    if (strcmp(serr, serr_save) != 0 &&
                        (ipl == SE_SUN || ipl == SE_MOON ||
                         ...
                         ipl == SE_CHIRON || ipl == SE_PHOLUS ||
                         ipl == SE_CUPIDO || ipl >= SE_AST_OFFSET ||
                ...
                if (strpbrk(fmt, "aADdQ") != NULL) {
                    ...
                    if (ipl == SE_FIXSTAR)
                        iflgret =
                            call_swe_fixstar(star, te, iflag2, xequ, serr);
                    else
                ...
                if (strpbrk(fmt, "IiHhKk") != NULL) {
                    ...
                    swe_azalt(tut, SE_EQU2HOR, geopos, datm[0], datm[1], xt,
                              xaz);
                    if (diff_mode) {
                ...
                if (strpbrk(fmt, "XU") != NULL) {
                    ...
                    if (ipl == SE_FIXSTAR)
                        iflgret =
                            call_swe_fixstar(star, te, iflag2, xcart, serr);
                    else
                ...
                if (strpbrk(fmt, "xu") != NULL) {
                    ...
                    if (ipl == SE_FIXSTAR)
                        iflgret =
                            call_swe_fixstar(star, te, iflag2, xcartq, serr);
                    else
                ...
                if (strpbrk(fmt, "gGj") != NULL) {
                    ...
                    if (hpos_meth >= 2 && toupper(ihsy) == 'G') {
                        swe_gauquelin_sector(tut, ipl, star2, iflag, hpos_meth,
                                             geopos, 0, 0, &hposj, serr);
                    }
                    else {
                        double cusp[100];
                        if (ihsy == 'i' || ihsy == 'I')
                            iflgret = swe_houses_ex(t, iflag, top_lat, top_long,
                                                    ihsy, cusp, cusp + 13);
                        hposj = swe_house_pos(armc, geopos[1], xobl[0], ihsy,
                                              xsv, serr);
                    }
                    if (toupper(ihsy) == 'G')
                        hpos = (hposj - 1) * 10;
                    ...
                    if (diff_mode) {
                        ...
                        if (hpos_meth == 1) xsv[1] = 0;
                        hpos2 = swe_house_pos(armc, geopos[1], xobl[0], ihsy,
                                              xsv, serr);
                        if (toupper(ihsy) == 'G')
                            hpos2 = (hpos2 - 1) * 10;
                        else
                ...
                if (iflgret < 0) {
                    ...
                    for (ipl = 1; ipl < iofs + 8; ipl++) {
                        ...
                        if (strpbrk(fmt, "IiHhKk") != NULL) {
                            ...
                            gpos[1] = top_lat;
                            gpos[2] = 0;
                            swe_azalt(t, SE_ECL2HOR, gpos, datm[0], datm[1], x,
                                      xaz);
                        }
                        if (strpbrk(fmt, "gGj") != NULL) {
                            hposj = swe_house_pos(armc, geopos[1], xobl[0],
                                                  ihsy, x, serr);
                            if (toupper(ihsy) == 'G')
                                hpos = (hposj - 1) * 10;
                            else
                                              ...
                                              int32 iflag, char *serr)
{
    ...
    if (0 && ipl == SE_MOON) {
        ...
        if ((retval =
                 swe_calc(tjd_et, SE_MOON, iflagi | SEFLG_J2000 | SEFLG_TRUEPOS,
                          xx, serr)) == ERR)
            return 0;
        dtrue = xx[2];
    }
    ...
    for (sp = fmt; *sp != '\0'; sp++) {
        ...
        switch (*sp) {
            ...
            case 'L':
                ...
                if (psp != NULL &&
                    (*psp == 'q' ||
                     *psp == 'y')) { /* delta t or time equation */
                    printf("%# 11.7f", x[0]);
                    printf("s");
                    break;
                }
            ...
            case 'n': {
                ...
                int imeth =
                    (*sp == tolower(*sp)) ? SE_NODBIT_MEAN : SE_NODBIT_OSCU;
                iflgret = swe_nod_aps(te, ipl, iflag, imeth, xasc, xdsc, NULL,
                                      NULL, serr);
            ...
            case 'f':
                if (!is_house) {
                    ...
                    int imeth =
                        (*sp == tolower(*sp)) ? SE_NODBIT_MEAN : SE_NODBIT_OSCU;
                    //	fprintf(stderr, "c=%c\n", *sp);
                    iflgret = swe_nod_aps(te, ipl, iflag, imeth, NULL, NULL,
                                          xper, xaph, serr);
            ...
            case 'v': {
                ...
                static int hexa[64] = {
                    1,  43, 14, 34, 9,  5,  26, 11, 10, 58, 38, 54, 61,
                    60, 41, 19, 13, 49, 30, 55, 37, 63, 22, 36, 25, 17,
                    21, 51, 42, 3,  27, 24, 2,  23, 8,  20, 16, 35, 45,
                    12, 15, 52, 39, 53, 62, 56, 31, 33, 7,  4,  29, 59,
                    40, 64, 47, 6,  46, 18, 48, 57, 32, 50, 28, 44};
                if (is_label) {
                    printf("hds");
                    break;
                }
                           ...
                           double *geopos, char *serr)
{
    ...
    while (special_event == SP_RISE_SET && tnext < t_ut + nstep) {
        ...
        if (last_was_empty && (star == NULL || *star == '\0')) {
            rval = swe_calc_ut(tnext, ipl, whicheph | SEFLG_EQUATORIAL, tret,
                               serr);
            if (rval >= 0) {
        ...
        while (!do_set && loop_count < 2) {
            rval = swe_rise_trans(tnext, ipl, star, whicheph, rsmi, geopos,
                                  datm[0], datm[1], &tset, serr);
            if (rval == ERR) {
                do_printf(serr);
                exit(0);
            }
        ...
        if (do_rise && do_set && trise > tset) {
            ...
            trise = 0;  // we hope that exact time 0 never happens, is highly
                        // unlikely.
        }
        if (do_rise && do_set) {
            ...
            // for sequences of days without rise or set, the line '-   -' is
            // printed only once.
    ...
    if (special_event == SP_MERIDIAN_TRANSIT) {
        ...
        for (ii = 0; ii < nstep; ii++, t_ut = tret1sv + 0.001) {
            ...
            if (swe_rise_trans(t_ut, ipl, star, whicheph, SE_CALC_MTRANSIT,
                               geopos, datm[0], datm[1], &(tret[0]),
                               serr) != OK) {
                do_printf(serr);
                return ERR;
            }
            /* transit over lower midheaven */
            if (swe_rise_trans(t_ut, ipl, star, whicheph, SE_CALC_ITRANSIT,
                               geopos, datm[0], datm[1], &(tret[1]),
                               serr) != OK) {
                do_printf(serr);
                return ERR;
            }
            ...
            if (tret[0] == 0 || tret[0] > tret[1])
                ...
                sprintf(sout + strlen(sout), "%2d.%02d.%04d\t%s    ", jday,
                        jmon, jyear, hms(jut, BIT_LZEROES));
            }
            if (have_gap_parameter) sprintf(sout + strlen(sout), "\t");
    ...
    for (ii = 0; ii < nstep; ii++, t_ut += direction) {
        *sout = '\0';
        ...
        if (special_mode & SP_MODE_HOW) {
            if ((eclflag = swe_lun_eclipse_how(t_ut, whicheph, geopos, attr,
                                               serr)) == ERR) {
                do_printf(serr);
                return ERR;
            }
        ...
        if (special_mode & SP_MODE_LOCAL) {
            ...
            t_ut = tret[0];
            rval =
                swe_calc_ut(t_ut, SE_MOON, whicheph | SEFLG_EQUATORIAL, xx, s1);
            if (rval < 0) strcat(s1, "\n");
            do_printf(s1);
            swe_revjul(t_ut, gregflag, &jyear, &jmon, &jday, &jut);
            ...
            sprintf(sout_short, "%s\t%2d.%2d.%4d%s\t%s\t%.3f\t%s\t%s\n", sout,
                    jday, jmon, jyear, sgj, hms(jut, 0), attr[8], s1, saros);
            sprintf(sout + strlen(sout),
                    "%2d.%02d.%04d%s\t%s\t%.4f/%.4f\tsaros %s\t%.6f\tdt=%.2f\n",
                    jday, jmon, jyear, sgj, hms(jut, BIT_LZEROES), attr[0],
                    attr[1], saros, t_ut,
                    swe_deltat_ex(t_ut, whicheph, serr) * 86400);
            /* second line:
             * eclipse times, penumbral, partial, total begin and end */
            if (have_gap_parameter) sprintf(sout + strlen(sout), "\t");
            sprintf(sout + strlen(sout), "  %s ", hms_from_tjd(tret[6]));
            ...
            if (special_mode & SP_MODE_HOCAL) {
                swe_split_deg(jut, SE_SPLIT_DEG_ROUND_MIN, &ihou, &imin, &isec,
                              &dfrc, &isgn);
                sprintf(sout, "\"%04d%s %02d %02d %02d.%02d %d\",\n", jyear,
                        sgj, jmon, jday, ihou, imin, ecl_type);
            }
        }
        ...
        if (with_chart_link) {
            ...
            sprintf(snat,
                    ...
                    saros, styp, jday, jmon, jyear, stim, cal, iflg, slon,
                    slat);
            sprintf(sout,
                    "<a "
                    "href='https://www.astro.com/cgi/"
                    "chart.cgi?muasp=1;nhor=1;act=chmnat;nd1=%s;rs=1;iseclipse="
                    "1' target='eclipse'>chart link</a>\n\n",
                    snat);
            do_printf(sout);
        }
    }
                                ...
                                double *geopos, char *serr)
{
    ...
    for (ii = 0; ii < nstep; ii++, t_ut += direction) {
        *sout = '\0';
         ...
         */
        if (special_mode & SP_MODE_LOCAL) {
            if ((eclflag = swe_sol_eclipse_when_loc(t_ut, whicheph, geopos,
                                                    tret, attr, direction_flag,
                                                    serr)) == ERR) {
                do_printf(serr);
                return ERR;
                ...
                if ((search_flag & SE_ECL_ANNULAR) &&
                    (eclflag & SE_ECL_ANNULAR)) {
                    strcpy(sout, "annular ");
                    has_found = TRUE;
                    ecl_type = ECL_SOL_ANNULAR;
                }
                if ((search_flag & SE_ECL_PARTIAL) &&
                    (eclflag & SE_ECL_PARTIAL)) {
                    strcpy(sout, "partial ");
                    has_found = TRUE;
                ...
                if (!has_found) {
                    ...
                    swe_calc(t_ut + swe_deltat_ex(t_ut, whicheph, serr),
                             SE_ECL_NUT, 0, x, serr);
                    if (time_flag & (BIT_TIME_LMT | BIT_TIME_LAT)) {
                        for (i = 0; i < 10; i++) {
                            if (tret[i] != 0) {
                                retc = ut_to_lmt_lat(tret[i], geopos,
                                                     &(tret[i]), serr);
                                if (retc == ERR) {
                                    do_printf(serr);
                                    return ERR;
                    ...
                    sprintf(
                        ...
                        jday, jmon, jyear, sgj, hms(jut, BIT_LZEROES), attr[8],
                        attr[0], attr[2], saros, t_ut);
	  ...
	  sprintf(sout + strlen(sout), "\t%d min %4.2f sec   %s %s %s %s", 
                ...
                strcpy(s2, hms(fmod(tret[4] + 0.5, 1) * 24, BIT_LZEROES)));
                    ...
                    if (have_gap_parameter)
                        insert_gap_string_for_tabs(sout, gap);
                    do_printf(sout);
                }
            }
        } /* endif search_local */
        ...
        if (!(special_mode & SP_MODE_LOCAL)) {
            if ((eclflag = swe_sol_eclipse_when_glob(
                     t_ut, whicheph, search_flag, tret, direction_flag,
                     serr)) == ERR) {
                do_printf(serr);
                return ERR;
            }
            ...
            if (with_chart_link) {
                ...
                sprintf(
                    ...
                    "href='https://www.astro.com/cgi/"
                    "chart.cgi?muasp=1;nhor=1;act=chmnat;nd1=%s;rs=1;iseclipse="
                    "1;topo=1' target='eclipse'>chart link</a>\n\n",
                    snat);
                do_printf(sout);
            }
        }
    ...
    for (ii = 0; ii < nstep; ii++) {
        *sout = '\0';
        ...
        if (special_mode & SP_MODE_LOCAL) {
            ...
            if ((eclflag = swe_lun_occult_when_loc(
                ...
                t_ut = tret[0] +
                       direction *
                           10; /* try again with start date increased by 10 */
                ii--;
            }
            else {
                t_ut = tret[0];
                if (time_flag & (BIT_TIME_LMT | BIT_TIME_LAT)) {
                    for (i = 0; i < 10; i++) {
                        if (tret[i] != 0) {
                            retc = ut_to_lmt_lat(tret[i], geopos, &(tret[i]),
                                                 serr);
                            if (retc == ERR) {
                                do_printf(serr);
                                return ERR;
                ...
                *sout = '\0';
                ...
                if (ipl != SE_SUN) {
                    if ((eclflag & SE_ECL_OCC_BEG_DAYLIGHT) &&
                        ...
                        strcat(sout, "(daytime)"); /* occultation occurs during
                                                      the day */
                    else if (eclflag & SE_ECL_OCC_BEG_DAYLIGHT)
                        strcat(sout, "(sunset) "); /* occultation occurs during
                                                      the day */
                    else if (eclflag & SE_ECL_OCC_END_DAYLIGHT)
                        strcat(sout, "(sunrise)"); /* occultation occurs during
                                                      the day */
                }
                if (have_gap_parameter) sprintf(sout + strlen(sout), "\t");
	  ...
	  sprintf(sout + strlen(sout), "\t%d min %4.2f sec   %s %s %s %s", 
        ...
        if (!(special_mode & SP_MODE_LOCAL)) {
            ...
            if (eclflag == 0) { /* no occltation was found at next conjunction,
                                   try next conjunction */
            ...
            if ((eclflag & SE_ECL_NONCENTRAL) && !(eclflag & SE_ECL_PARTIAL))
                strcat(sout, "non-central ");
            t_ut = tret[0];
            swe_lun_occult_where(t_ut, ipl, star, whicheph, geopos_max, attr,
                                 serr);
            /* for (i = 0; i < 8; i++) {
              printf("attr[%d]=%.17f\n", i, attr[i]);
            } */
            ...
            if (!(eclflag & SE_ECL_PARTIAL) && !(eclflag & SE_ECL_NONCENTRAL)) {
                if ((eclflag = swe_lun_occult_when_loc(
                         t_ut - 10, ipl, star, whicheph, geopos_max, tret, attr,
                         0, serr)) == ERR) {
                    do_printf(serr);
                    return ERR;
                }
    ...
    if (event_type <= 4) {
        if (hel_using_AV) {
            ...
            sprintf(sout + strlen(sout), "%s %s: %d/%02d/%02d %s %s (%.5f)\n",
                    obj_name, sevtname[event_type], jyear, jmon, jday, stim0,
                    stz, dret[0]);
        }
        else {
            ...
            sprintf(sout + strlen(sout),
                    "%s %s: %d/%02d/%02d %s %s (%.5f), opt %s, end %s, dur "
                    "%.1f min\n",
                    obj_name, sevtname[event_type], jyear, jmon, jday, stim0,
                    stz, dret[0], stim1, stim2, (dret[2] - dret[0]) * 1440);
        }
    }
    else {
        strcpy(stim0, hms_from_tjd(dret[0]));
                                 ...
                                 double *dobs, char *serr)
{
    ...
    for (ii = 0; ii < nstep; ii++, t_ut = dret[0] + 1) {
        *sout = '\0';
        ...
        retflag = swe_heliacal_ut(t_ut, geopos, datm, dobs, obj_name,
        ...
        if (search_flag == 0) {
            if (ipl == SE_VENUS || ipl == SE_MERCURY) {
                 ...
                 * morning appearances in betweeen. We have to find out when the
                 * next morning appearance is and then find all evening
                 * appearances that take place before that */
                if (ipl == SE_MERCURY) {
                    event_type = SE_HELIACAL_RISING;
                    retflag =
                        swe_heliacal_ut(dret[0], geopos, datm, dobs, obj_name,
                                        event_type, helflag, dret, serr);
                    if (retflag == ERR) {
                        do_printf(serr);
                        return ERR;
...
#if MSDOS
    {
        ...
        *s = '\0';
        ...
        for (i = 0; i < np; i++) {
            ...
            for (j = 0; j < 3; j++) {
                if (sp[j] != NULL &&
                    strlen(path) + 2 + strlen(s) < AS_MAXCH - 1) {
                    sprintf(path + strlen(path), "%c:%s%c", *sp[j], s,
                            *PATH_SEPARATOR);
                }
            }
        }
...
static void jd_to_time_string(double jut, char *stimeout)
{
    ...
    t2 = (t2 - (int32)t2) * 1000;
    if ((int32)t2 > 0) {
        sprintf(stimeout + strlen(stimeout), ".%03d",
                (int)t2);  // millisec, if > 0
    }
}
